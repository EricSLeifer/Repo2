...
#' n <- 4600
#' power13_13_13(n, hrA, hrB, hrAB, dig, avgprob, probA_C, probAB_C,
#'   crit13, cormat12 = matrix(c(1, sqrt(0.5), sqrt(0.5), 1), byrow = TRUE,
#'   nrow = 2), cormat23 = matrix(c(1, 0.5, 0.5, 1), byrow = TRUE, nrow = 2),
#'   cormat123 = matrix(c(1, sqrt(0.5), sqrt(0.5), sqrt(0.5), 1, 0.5,
#'   sqrt(0.5), 0.5, 1), byrow=TRUE, nrow = 3), niter = 100, rseed = 047477, abseps = 1e-05)
#'
#' # $poweroverA
#' # [1] 0.5861992
#'
...
#' # [1] 0.9302084
#'
power13_13_13 <- function(n, hrA, hrB, hrAB, dig, avgprob, probA_C, probAB_C,
                          crit13,
                          cormat12 = matrix(c(1, sqrt(0.5),
                                ...
                                niter = 100, rseed = 047477, abseps = 1e-05)
{
  ...
  for(i in 1:niter){
    powermat[i, 1] <- pmvnorm(lower=-Inf, upper=c(crit13, crit13), mean=c(muoverA, muA),
                          corr=cormat12, sigma=NULL, maxpts = 25000, abseps = abseps,
                          releps = 0)
    ...
    powermat[i, 3] <- pmvnorm(lower=-Inf, upper = c(crit13, crit13), mean = c(muA, muAB),
                          corr=cormat23, sigma=NULL, maxpts = 25000, abseps = abseps,
                          releps = 0)

    powermat[i, 4] <- pmvnorm(lower=-Inf, upper=c(crit13, crit13, crit13), mean=c(muoverA, muA, muAB),
                           corr=cormat123, sigma=NULL, maxpts = 25000, abseps = abseps,
                           releps = 0)
  }
